  ...
  <div>
    ...
    <div id="header" class="header navbar-inverse">
      ...
      <div class="navbar-header">
        <button type="button" class="navbar-toggle pull-left" v-on:click="toggleMobileRightSidebar"
          v-if="pageOptions.pageWithTwoSidebar">
          <span class="icon-bar"></span>
          <span class="icon-bar"></span>
          <span class="icon-bar"></span>
        </button>
        <router-link to="/dashboard" class="navbar-brand"><span class="navbar-logo"></span> <b>Digital</b> Report
        </router-link>
        ...
        <button type="button" class="navbar-toggle" v-on:click="toggleMobileTopMenu"
          v-if="pageOptions.pageWithTopMenu && pageOptions.pageWithoutSidebar">
          <span class="icon-bar"></span>
          <span class="icon-bar"></span>
          <span class="icon-bar"></span>
        </button>
        <button type="button" class="navbar-toggle p-0 m-r-0" v-on:click="toggleMobileMegaMenu"
      ...
      <div class="navbar-nav navbar-right">
        <router-link to="#" v-b-modal.loginDialog class="navbar-brand" style="font-size: 15px"
          v-if="this.$store.state.userdata == null"><b>Login</b>
        </router-link>
        ...
        <b-nav-item-dropdown menu-class="dropdown-menu-right" class="dropdown navbar-user" no-caret
          ...
          <template slot="button-content">
            ...
            <span class="d-none d-md-inline">{{this.$store.state.userdata.txtUsername}}</span> <b class="caret"></b>
          </template>
    ...
    <b-modal id="loginDialog" hide-footer cancel-variant="default" title="Login" @show="resetModal"
      @hidden="resetModal">
      <form @submit.stop.prevent="login" method="POST" class="margin-bottom-0">
        <div class="form-group m-b-20">
          <input type="text" class="form-control form-control-lg inverse-mode" placeholder="Username" v-model="username"
            required />
        </div>
        <div class="form-group m-b-20">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      setAuthLogin(user) {
        this.$axios.get('/user/' + user.id).then(response => {
